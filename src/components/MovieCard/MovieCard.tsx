...
const MovieCard: React.FC<{movie: Movie, onDetails: (ID_Movie: number) => void}> = ({movie, onDetails}) => {
    ...
    return (
        <div 
            key={movie.ID_Movie} 
            className="rounded-lg p-4 bg-black/5 border-2 border-solid border-black/5 transition-all hover:bg-black/10 flex flex-col items-stretch justify-start gap-2"
            onClick={() => onDetails(movie.ID_Movie)}>
